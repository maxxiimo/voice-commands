...
# Octave -------------------------------------------------------------------------------------
        ...
        # Matrices
            ...
            # Generators
            "[octave] generate eye <n> [identity matrix]":       Text(" = eye(%(n)d)") + Key("home"),                       # <variable> = eye(<rows_columns>)                       | Generates a <n> by <n> identity matrix.
            "[octave] generate <n> by [X] ones matrix":          Text(" = ones(%(n)d,)") + Key("left"),                     # <variable> = ones(<rows>,<columns>)                    | Generates a matrix that is comprised of all ones and the dimensions specified.
            "[octave] generate <n> by [X] zeros matrix":         Text(" = zeros(%(n)d,)") + Key("left"),                    # <variable> = zeros(<rows>,<columns>)                   | Generates a matrix that is comprised of all zeros and the dimensions specified.
            "[octave] generate <n> by [X] random matrix":        Text(" = rand(%(n)d,)") + Key("left"),                     # <variable> = rand(<rows>,<columns>)                    | Generates a matrix that is comprised of all random numbers uniformly drawn between 0 and 1 by the dimensions specified.
            "[octave] generate <n> by [X] normal random matrix": Text(" = randn(%(n)d,)") + Key("left"),                    # <variable> = randn(<rows>,<columns>)                   | Generates a matrix that is comprised of all random numbers from Gaussian distribution with mean 0 and variance of standard deviation equal to 1.
            # Delete
